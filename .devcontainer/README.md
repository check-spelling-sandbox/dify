...
a simple workaround is change `/signin` endpoint into another one, then log in with GitHub account and close the tab, then change it back to `/signin` endpoint. Then all things will be fine.
The reason is [`signin` endpoint is not allowed in codespaces](https://github.com/orgs/community/discussions/5204)
